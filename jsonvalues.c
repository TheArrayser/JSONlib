...
JSON_RET JSONThingToStringRecursivlyinternal(JSON_THING_t object, char * * out, unsigned int * currentBuf, unsigned int * totalAllocatedminusOne){
    ...
    switch (object->type){
    case JSON_e_BOOLEAN:printf("BOOLEAN\n");
        {
    ...
    case JSON_e_STRING:printf("STRING\n");
        {
    ...
    case JSON_e_DOUBLE:printf("DOUBLE\n");
        {
    ...
    case JSON_e_INTEGER:printf("INTEGER\n");
        {
    ...
    case JSON_e_OBJECT:printf("OBJECT\n");
        {
            ...
            for(int i = 0; i < maplength; i++){
                ...
                *currentBuf += keystringLength;
                ...
                (*out)[*currentBuf] = ':';
                (*currentBuf)++;
                JSON_THING_t newthing = getElementByIndex(object->thing.pointer, i);
                JSON_RET possibleRet = JSONThingToStringRecursivlyinternal(newthing, out, currentBuf, totalAllocatedminusOne);
                if(possibleRet){
                    return possibleRet;
                }
                if(i != maplength - 1){
    ...
    case JSON_e_ARRAY: printf("ARRAY\n");
        {
